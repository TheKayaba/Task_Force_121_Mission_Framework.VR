...
[{
    if (!isNil "m_isSpectator") exitWith {diag_log "fn_CamInit: Already ran, exiting early."};
    ...
    if (isNil "m_spect_playerGroup") then {m_spect_playerGroup = _newGrp;m_spect_playerGroupNumber = 1;};

    switch (side m_spect_playerGroup) do {
        case WEST: {m_spect_newUnit = m_spect_playerGroup createUnit ["B_VirtualCurator_F", _pos, [], 0, "FORM"];};
        case EAST: {m_spect_newUnit = m_spect_playerGroup createUnit ["O_VirtualCurator_F", _pos, [], 0, "FORM"];};
        case INDEPENDENT: {m_spect_newUnit = m_spect_playerGroup createUnit ["I_VirtualCurator_F", _pos, [], 0, "FORM"];};
        case CIVILIAN: {m_spect_newUnit = m_spect_playerGroup createUnit ["C_VirtualCurator_F", _pos, [], 0, "FORM"];};
        default {m_spect_newUnit = m_spect_playerGroup createUnit ["VirtualCurator_F", _pos, [], 0, "FORM"];};
    };
    m_spect_newUnit allowDamage false;
    selectPlayer m_spect_newUnit;

    // ====================================================================================
    // Wait until player becomes the new unit
    [{player isEqualTo m_spect_newUnit}, {
        params ["_unit", "_oldUnit"];

        [m_spect_newUnit] call m_fnc_disableCSpect;
        deleteVehicle _unit;
	...
	f_cam_checkAcreMute = {
	    ...
	    //["f_spect_tags", "onEachFrame", {_this call F_fnc_DrawTags}] call BIS_fnc_addStackedEventHandler;
            ...
            [{
                ...
                player joinAsSilent [m_spect_playerGroup, m_spect_playerGroupNumber];
            }, []] call CBA_fnc_execNextFrame;

            // Add player to the spectator list kept on the server
            m_spectatorPV = player;
            publicVariableServer "m_spectatorPV";
            player setVariable ["m_isUnitSpectator",true,true];
