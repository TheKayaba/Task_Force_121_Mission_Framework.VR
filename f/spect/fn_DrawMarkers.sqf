...
{
    private _isSpectator = _x getVariable ["m_isUnitSpectator",false];
    if (alive _x && {!_isSpectator}) then {
        ...
        if (leader _x isEqualTo _x && {{private _isSpectator = _x getVariable ["m_isUnitSpectator",false]; (isPlayer _x) && (!_isSpectator)} count units _x > 0}) then {
...
if (!isNil "m_sector_triggerArray") then {
    {
        private _owner = _x getVariable "m_sector_lastOwner";
