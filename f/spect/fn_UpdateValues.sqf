...
f_cam_updateValues = [{
    ...
    {
        if (!(_x in f_cam_listUnits) && ({ private _spectator = _x getVariable ["m_isUnitSpectator",false]; (alive _x) && {!_spectator} } count units _x) > 0 ) then {
            ...
            {
                if (alive _x) then {
                    private _spectator = _x getVariable ["m_isUnitSpectator",false];
    ...
    private _listUnits = f_cam_listUnits;
    {
        ...
        if (_index >= 0) then {
            if (typeName _x isEqualTo "GROUP") then {
                private _count = ({private _spectator = _x getVariable ["m_isUnitSpectator",false]; !_spectator && {alive _x}} count units _x);
                ...
                if (!alive _x || isNull _x || _x getVariable ["m_isUnitSpectator",false]) then {
                    lbDelete [_listBox,_index];
                    f_cam_listUnits deleteAt _index;
                    [] call f_cam_checkIndex;
                };
