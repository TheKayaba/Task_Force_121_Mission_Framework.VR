...
if (_bluforEliminatedMessage) then {
    _aliveB = {_isSpectator = _x getVariable ["m_isUnitSpectator",false]; side group _x isEqualTo west && !_isSpectator} count allUnits;
    if (_aliveB isEqualTo 0) then {_sideArray set [0,1];} else {_sideArray set [0,0];};
};
if (_redforEliminatedMessage) then {
    _aliveR = {_isSpectator = _x getVariable ["m_isUnitSpectator",false]; side group _x isEqualTo east && !_isSpectator} count allUnits;
    if (_aliveR isEqualTo 0) then {_sideArray set [1,1];} else {_sideArray set [1,0];};
};
if (_greenforEliminatedMessage) then {
    _aliveG = {_isSpectator = _x getVariable ["m_isUnitSpectator",false]; side group _x isEqualTo independent && !_isSpectator} count allUnits;
