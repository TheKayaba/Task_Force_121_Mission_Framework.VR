...
publicVariable "m_hasGameStarted";
...
/*"m_JIP_playerConnected" addPublicVariableEventHandler {
    ...
    if (isNil "m_spect_spectatorList") then {m_spect_spectatorList = [];};
    [{
...
"m_spectatorPV" addPublicVariableEventHandler {
    ...
    if (isNil "m_spect_spectatorList") then {m_spect_spectatorList = [];};
    m_spect_spectatorList pushBack _spectator;
};
m_spect_serverPVEHAdded = true;
publicVariable "m_spect_serverPVEHAdded";
