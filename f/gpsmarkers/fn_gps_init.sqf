...
Vehicle Markers
    ...
    If you want the vehicle to have a custom name instead of something weird looking like 'bluTruck' then add the following line to the vehicles initialization field in the editor to change the GPS marker name to whatever you want:
        ...
        this setVariable ["m_MarkerName", "GPS Marker Text Goes Here"];
    
Excluding groups from being marked
    By default, all groups will have markers generated for them even if their leader doesn't have GPS. If you wish to exclude a group from having a marker generated and updated, add it's groupID to the array m_ignoreMarkerArray on line 51. In the default template all groups have their groupIDs set by the file "f\setGroupID\f_setGroupID.sqf" so you will want to use the values defined there. Make sure when you enter a groupID you surround it with quotes since it is a string. 
    
    For example, if I wanted to exclude groups with the groupIDs "P", "AV", "BV", "CV" and "DV" from having markers created I would set m_ignoreMarkerArray to:

    m_ignoreMarkerArray = ["P","AV","BV","CV","DV"];
    
    To make sure all groups have markers set m_ignoreMarkerArray to be an empty array. If you have just downloaded the template off GITHUB then this should be the default setting. To do this use the following line:
...
_useMarkers = ["m_gps_markersEnabledParam",1] call BIS_fnc_getParamValue;
if (_useMarkers isEqualTo 0) exitWith {};

#include "settings.sqf";

switch (side player) do {
    case west: { if (!isNil "_westVehArray") then {m_sideVehArray = _westVehArray;} else {m_sideVehArray = nil;}; };
    case east: { if (!isNil "_eastVehArray") then {m_sideVehArray = _eastVehArray;} else {m_sideVehArray = nil;}; };
    case independent: { if (!isNil "_indVehArray") then {m_sideVehArray = _indVehArray;} else {m_sideVehArray = nil;}; };
    default { m_sideVehArray = nil; };
};
if (!isNil "m_sideVehArray") then {
    m_sideVehArray2 = [];
    {
        _veh = _x select 0;
        _ident = _x select 1;
        m_sideVehArray2 pushBack _veh;
    } forEach m_sideVehArray;
};
if (!isNil "_sidesVisibleToWest" && {!isNil "_sidesVisibleToEast"} && {!isNil "_sidesVisibleToGuer"}) then {
    ...
    switch (side player) do {
        case west: { m_sidesVisibleToPlayer = _sidesVisibleToWest; };
        case east: { m_sidesVisibleToPlayer = _sidesVisibleToEast; };
        case independent: { m_sidesVisibleToPlayer = _sidesVisibleToGuer; };
        default { m_sidesVisibleToPlayer = []; };
    };
};
call m_fnc_gps_createMarks;