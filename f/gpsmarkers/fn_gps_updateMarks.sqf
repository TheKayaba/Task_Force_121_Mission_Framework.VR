fn_m_updateVehMarks={
    private _vehArray = _this select 0;
    {
        ...
        if ("ItemGPS" in (assignedItems player)) then {
            private _unitInside = _veh getVariable "m_UnitInside";
            ...
            if (!isNil "_unitInside") then {
                private _markerText = (_veh getVariable "m_MarkerName") + " (" + (_veh getVariable "m_UnitInside") + ")";
                _veh setVariable ["m_UnitInside",nil];
                _veh setVariable ["m_LastInside",nil];
                _marker setMarkerTextLocal _markerText;
            } else {
                private _markerText = (_veh getVariable "m_MarkerName");
                _marker setMarkerTextLocal _markerText;
            };
...
{ //forEach allGroups
    private _group = _x;
    private _groupIdent = _group getVariable ["m_groupIdentifier",groupID _group];
    if !( ((_groupIdent) in m_ignoreMarkerArray) || {((groupID _group) in m_ignoreMarkerArray)} ) then {
        private _marker = _group getVariable "m_gps_markerName";
        private _marker2 = _marker + "Size";
        if ("ItemGPS" in (assignedItems player)) then { //Check if player has GPS
            if ((side (leader _x)) in m_sidesVisibleToPlayer) then { //Check if player is supposed to be able to see this group
                ...
                { //forEach allUnits in _group
                    ...
                    private _unit = _x;
                    private _senior = _group getVariable ["m_seniorGPS",objNull];
                    if (!("ItemGPS" in (assignedItems _senior)) || (!alive _senior) || !(group _senior isEqualTo _group)) then { //Make sure most senior unit still has GPS and is alive
                        _group setVariable ["m_seniorGPS",objNull];
                        _senior = objNull;
                    };
                    ...
                    if (_senior != leader _group) then {
                        if ("ItemGPS" in (assignedItems (leader _group)) && {alive (leader _group)} && {(group (leader _group)) isEqualTo _group} ) then {
                            ...
                            _group setVariable ["m_seniorGPS",(leader _group)];
                        };
                    };
                    
                    //_senior = _group getVariable ["m_seniorGPS",objNull]; // put this here or _senior won't update for the below code block
                    if (("ItemGPS" in (assignedItems _unit)) && {(isNull _senior) || (_senior isEqualTo _unit)}) then { //no reason to run this stuff if _unit isn't the most senior member
                        if (isNull _senior) then {
                            _group setVariable ["m_seniorGPS",_unit]; //no better match than _unit
                        };
                        if (vehicle _unit != _unit) then { //If unit isn't on foot and vehicle doesn't have a marker
                            private _unitInside = (vehicle _unit) getVariable "m_UnitInside";
                            private _lastInside = (vehicle _unit) getVariable "m_LastInside";
                            if (isNil "_lastInside") then {_lastInside = "Nobody"};
                            if ((!isNil "_unitInside") && (_lastInside != _groupIdent)) then {
                                private _string = ((vehicle _unit) getVariable "m_UnitInside") + " | " + (groupID _group);
                                (vehicle _unit) setVariable ["m_UnitInside",_string];
                                (vehicle _unit) setVariable ["m_LastInside",_groupIdent];
                            } else {
                                (vehicle _unit) setVariable ["m_UnitInside",(groupID _group)];
                                (vehicle _unit) setVariable ["m_LastInside",_groupIdent];
                            };
                            if (!isNil "m_sideVehArray") then {
                                if (vehicle _unit in m_sideVehArray2) then {
...
//VEHICLE MARKERS
if (!isNil "m_sideVehArray") then {[m_sideVehArray] call fn_m_updateVehMarks;};