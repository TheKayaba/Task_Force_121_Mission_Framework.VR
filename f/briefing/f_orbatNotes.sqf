...
_generateORBAT = {
    ...
    // Loop through the group, print out group ID, leader name and medics if present
    {
        if !((count units _x) isEqualTo 0) then {
            ...
            private _groupSize = _x getVariable ["m_gps_groupSize",0];
            private _name = groupID _x;
            private _longName = _x getVariable ["m_LongName",groupID _x];
            ...
            if (_changeColor) then {
                if (isNil "m_orbat_lastUsedColor") then {
                    m_orbat_lastUsedColor = ["#FFFFFF"];
                };
                m_colorArray = (m_colorArrayBase - m_colorArrayUsed);
                if (count m_colorArray isEqualTo 0) then {
                    m_colorArrayUsed = [];
                    m_colorArray = m_colorArrayBase;
                };
                _color = m_colorArray select 0;
                m_colorArrayUsed pushBack _color;
                m_orbat_lastUsedColor = [_color];
            } else {
                if (isNil "m_orbat_lastUsedColor") then {
                    m_orbat_lastUsedColor = ["#FFFFFF"];
                };
                ...
                _color = m_orbat_lastUsedColor select 0;
            };

            switch (f_param_radios) do {
              case 1: { // TFR
                // Get group's radio frequency
                private _freq = _x getVariable ["m_radioSettings",nil];
                if (isNil "_freq") then {
                    _freq = "UNK";
                } else {
                    ...
                    if (!(_freq isEqualTo "UNK")) then {
                        _freq = _freq + m_playerBaseChannel;
                    };
                };
...
waitUntil { !isNil "m_groupIDset" };
switch (f_param_radios) do {
  case 1: { // TFR
    waitUntil { !isNil "m_playerBaseChannel" };
  };
  case 2: { // acre2
    waitUntil { !isNil "m_acre_setup"; };
  };
...

{
    ...
    private _identity = _x getVariable ["m_groupIdentifier",groupID _x];
...
private _color = m_colorArray select 0;
m_colorArrayUsed pushBack _color;
m_orbat_lastUsedColor = [_color];
